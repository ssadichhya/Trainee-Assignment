...
def get_employees(skip: int, limit: int):
    ...
    try:
        cursor.execute(
            "SELECT * FROM employees LIMIT ? OFFSET ?", (limit, skip))
        rows = cursor.fetchall()
        employees = [Employee(id=row[0], name=row[1],
                              department=row[2]) for row in rows]
        return employees
    finally:
        conn.close()
...
def update_employee(employee_id: int, column: str, new_value: str):
    ...
    try:
        cursor.execute(
            f"UPDATE employees SET {column} = ? WHERE id = ?", (new_value,
                                                                employee_id))
        conn.commit()
        return cursor.rowcount > 0
    finally:
